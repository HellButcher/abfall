...
unsafe impl Trace for Node {
    fn trace(&self, tracer: &Tracer) {
        if let Some(n) = &self.next { tracer.mark(n); }
    }
}
...
fn root_preservation_after_collection() {
    ...
    for _ in 0..1000 { let _tmp = ctx.allocate(0usize); } // many temporaries
...
fn sweep_frees_memory() {
    ...
    for _ in 0..500 { let _t = ctx.allocate(vec![0u8; 256]); } // allocate ~128KB
...
fn threshold_triggers_collection() {
    ...
    for _ in 0..200 { let _t = ctx.allocate([0u8; 64]); }
...
fn tracing_chain_keeps_all_nodes() {
    ...
    let head = ctx.allocate(Node { value: 0, next: None });
    ...
    loop {
        count += 1;
        if let Some(next_ptr) = cur.next { // cur derefs to &Node via GcRoot
    ...
    for _ in 0..4 {
        ...
        handles.push(thread::spawn(move || {
            ...
            for i in 0..10_000 {
                ...
                if i % 50 == 0 { ctx.heap().collect(); }
                if *r != i { panic!("value corruption"); }
            }
        }));
    }
    // Simultaneously trigger collections
    for _ in 0..50 { heap.collect(); thread::yield_now(); }
...
fn write_barrier_concurrent_mutation() {
    ...
    for _ in 0..2000 { let _ = ctx.allocate([0u8; 16]); }
    ...
    for _ in 0..4 {
        ...
        handles.push(thread::spawn(move || {
            let ctx = GcContext::with_heap(heap_cl);
            for v in values_cl { cell_ptr.set(v); }
        }));
    }
    for h in handles { h.join().unwrap(); }
    // Force completion
...
fn large_object_graph_survives_multiple_cycles() {
    ...
    for cycle in 0..5 {
        for _ in 0..500 { let _t = ctx.allocate([0u8; 64]); }
        ctx.heap().force_collect();
        for (i, r) in roots.iter().enumerate() { assert_eq!(r[0], i); }
